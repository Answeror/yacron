...
class CronField(object):
    ...
    def __init__(self, *args):
        ...
        if isinstance(self.value, int):                     #numbers must be within bounds
            if not self.min <= self.value <= self.max:
                raise ValueError('Value must be between {0} and {1}'.format(self.min, self.max))
        if isinstance(self.value, str):                      #sub name/abbr for month/dayofweek
    ...
    def __contains__(self, item):
        ...
        value = self.value
        if isinstance(value, int):                          #standard numeric (python obj)
            return value == item
...
class CronExpression(object):
    ...
    FIELDS = dict(list(zip(FIELD_NAMES, (sec, min, hr, dom, mon, dow, yr))))                      #field name->init func
    ...
    def __init__(self, **kwargs):
        ...
        expression = dict(list(zip(self.FIELD_NAMES, expression.split())))
        for field, ctor in list(self.FIELDS.items()):
    ...
    def __contains__(self, item): #item should always be a datetime #XXX confirm and revise
        if not isinstance(item, datetime.datetime): #hrm
            return False
        item = dict(list(zip(self.STRUCT_TIME, item.timetuple()[:7])))
        return all(item[k] in v for k,v in list(self.__dict__.items()))
    ...
    def __eq__(self, other):
        ...
        if not isinstance(other, CronExpression):
            return False
        return all(getattr(other, k) == v for (k,v) in list(self.__dict__.items()))
...
class CronTab(threading.Thread):
    ...
    def run(self):
        ...
        try:
            ...
            for job in (self.jobs.pop(k) for (k,v) in list(self.jobs.items()) if v.reboot):
            ...
            while True:
                ...
                for _, job in list(self.jobs.items()):
                    if now in job.cron:
                        self.CONTEXTS[job.ctx](job)
...
def job(*args, **kwargs):
    ...
    fkwargs = dict((k, kwargs[k]) for k in list(kwargs.keys()) if k not in CronExpression.FIELD_NAMES) #func specific kwargs
